...
const KeyboardScene = ({ maxDpr }: { maxDpr: number }) => {
  ...
  useEffect(() => {
    ...
    const onCustomSelect = (e: Event) => {
      const customEvent = e as CustomEvent<Skill>;
    ...
    window.addEventListener("portfolio:select-skill", onCustomSelect);

    return () => {
      window.removeEventListener("portfolio:select-skill", onCustomSelect);
